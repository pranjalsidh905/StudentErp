...
export const uploadCSVFile = (file) => async (dispatch) => {
  try {
    dispatch(uploadCsvRequest());

    const formData = new FormData();
    // Append each property of the 'file' object to FormData
    Object.keys(file).forEach((key) => {
      formData.append(key, file[key]);
    });

    const { data } = await api.uploadCSVFile(formData);
    dispatch(uploadCsvSuccess(data));
    alert("CSV file uploaded successfully");
  } catch (error) {
    console.error("Error uploading CSV file:", error);
    console.error("Error response from server:", error.response?.data);    
    dispatch(uploadCsvFailure(error));
